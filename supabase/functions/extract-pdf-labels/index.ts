...
const SKU_PATTERN = /\b(B|DB|SB|CB|EB|LS|LSB|W|UB|WC|OH|BLW|BRW|T|TF|UT|TC|PT|PTC|UC|V|VB|VD|VDC|FIL|BF|WF|BFFIL|WFFIL|TK|TKRUN|CM|LR|EP|FP|DWR|HA|HAV|HALC|HAL|SA|SV|APPRON)\d[\w\-\/]*(?:\((?:SPLIT)\)|\[(?:SPLIT)\]|_SPLIT)?/gi;
...
const STRONG_STRIP_SKU_RE = /^(?:UC|BP|SCRIBE|APPRON|[A-Z]{2,8}\d[A-Z0-9\-\/]{2,})$/i;
...
function classifySku(sku: string): string {
  ...
  if (/^(FIL|BF|WF|BFFIL|WFFIL|TK|TKRUN|CM|LR|EP|FP|DWR|TF|APPRON)\d/i.test(sku)) return "Accessory";
  return "Base";
}
